...
"""
    Hangman Game
    * a dict of words where for each start, we use a random word and the player has to guess it right and if he cant he will eventually lost after a max of 6 attempts *
    -
...
class Hangman:
    def __init__(self):
        """
            El layout es una lista madre que abarca listas añidadas donde cada una representa visualmente una linea en el interface del juego. Por consiguiente para contextualizar tanto el dibujo, las letras, la palabra a adivinar y los botones procedemos de la siguiente manera
        ...
        layout = [
            ...
            ], # para representar el dibujo y las letras
            [
                self._build_guessed_word_frame(),
            ], # representar la palabra a adivinar
            [
                self._build_action_buttons_frame(),
            ], # los botones del juego
        ]
        self._window = sg.Window(title="Hangman Game", layout=layout, finalize=True)# finalize True para congelar la interface del juego mientras el jugador este jugando sino no vamos a pooder realizar modificaciones en tiempo real
    ...
    def _build_letters_frame(self):
        ...
        letter_buttons = [
            [
                ...
                for letter in letter_group # ABCD
            ]
    ...
    def _select_word(self):
        # connect to the text file to read it line by line and select a random word eliminating any non essential spaces
        with open("hangman_words.txt", mode="r", encoding="utf-8") as words:
            word_list = words.readlines()
        return choice(word_list).strip().upper()

    def _build_guessed_word(self):
        # check for each letter in the hidden word if the input of the player match one of this letter we add it to the current letter list if not we use the astheric ( that will be the initial format for the game)
        current_letters = []
    ...
    def _new_game(self):
        # initialize the game with a new word being selected randomly from the txt file and restart the game
    ...
    def _restart_game(self):
        ...
        # Restart GUI
        self._canvas.erase() # erase method allows us to start from scratch with the hangman design
        self._draw_scaffold()
    ...
    def _play(self, letter):
        """
        if we got one letter right, we add it in the * current letter* and since we used a set above we 
        somehow avoiding duplicating letter so whenever we hit a letter and this letter has more than one ocurrence in the word we reveal all once.. if we dont get it right we draw a little piece of the hangman
        """
